...
appsettings.json: Fx9600.IpAddress = "auto"
  → DeviceDiscoveryService.DiscoverAsync()
    → TCP probe :5084 (LLRP) en subnets comunes 192.168.100.x / 192.168.1.x / 192.168.0.x
    → Ping sweep en subnet del adaptador activo
    → mDNS (Zeroconf) busca "FX9600"
    → TCP probe :5084 (LLRP) + :80 (HTTP) sobre candidatos
...
RfidReaderService
    ...
    └── Modo real (--no-sim): RFIDReader → TCP → FX9600:5084
            ...
            ├── Detectar antenas conectadas → GetPhysicalProperties().IsConnected
            ├── Configurar solo antenas físicamente conectadas
